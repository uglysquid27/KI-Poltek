...
{
    // Get the authenticated user using cookie-based authentication
    $token = $request->cookie('auth_token');
    $authenticatedUser = null;
    ...
    if ($validator->fails()) {
        Log::warning('Validation failed for Desain Industri submission: ' . json_encode($validator->errors()->all()));
        return back()
    ...
    Log::info('Validated data received. Keys: ' . implode(', ', array_keys($validatedData)));
    
    // Debug: Check if specific fields exist
    if (!isset($validatedData['judul_desain'])) {
        Log::error('judul_desain is missing from validated data!');
        Log::error('Available keys: ' . json_encode(array_keys($validatedData)));
    }
    ...
    // Prepare Desain Industri data - IMPORTANT: DO NOT include user_id
    $desainIndustriData = [
        'ki_id' => $kekayaanIntelektual->ki_id,
        // Do NOT include 'user_id' here - it's not in the desain_industris table
        'judul_desain' => $validatedData['judul_desain'],
        'kegunaan' => $validatedData['kegunaan'],
        'klaim_desain' => json_encode($validatedData['klaim_desain']),
        ...
        'anggota_pendesain' => isset($validatedData['anggota_pendesain']) ? json_encode($validatedData['anggota_pendesain']) : null,
        'file_path_gambar_desain' => $filePaths['gambar_desain'],
        'keterangan_gambar' => isset($validatedData['keterangan_gambar']) ? json_encode($validatedData['keterangan_gambar']) : null,
