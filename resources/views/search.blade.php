            ...
            <form action="{{ route('advancedSearch') }}" method="GET" class="space-y-4 text-gray-500">
                ...
                <div class="flex md:flex-row flex-col md:space-x-4 space-y-4 md:space-y-0">
                    <div class="w-full md:w-1/2">
                        <label for="type_filter" class="block mb-1 font-medium text-gray-700 text-sm">Jenis Kekayaan Intelektual:</label>
                        <select name="type_filter" id="type_filter"
                                class="px-4 py-2 border input-bordered rounded-lg focus:ring-0 w-full input">
                            <option value="">Pilih Jenis</option>
                            <option value="hak_cipta" {{ request('type_filter') == 'hak_cipta' ? 'selected' : '' }}>Hak Cipta</option>
                    ...
                    <div class="w-full md:w-1/2">
                        ...
                        <select name="status" id="status"
                            ...
                            <option value="Ditarik kembali" {{ request('status') == 'Ditarik kembali' ? 'selected' : '' }}>Ditarik kembali</option>
                ...
                <div class="gap-4 grid grid-cols-1 md:grid-cols-2">
                    <div>
                        <label for="submission_date_from" class="block mb-1 font-medium text-gray-700 text-sm">Tanggal Pengajuan (Dari):</label>
                        <input type="date" name="submission_date_from" id="submission_date_from"
                               class="px-4 py-2 border input-bordered rounded-lg focus:ring-0 w-full input"
                               value="{{ request('submission_date_from') }}" />
                    </div>
                    <div>
                        <label for="submission_date_to" class="block mb-1 font-medium text-gray-700 text-sm">Tanggal Pengajuan (Sampai):</label>
                        <input type="date" name="submission_date_to" id="submission_date_to"
                               class="px-4 py-2 border input-bordered rounded-lg focus:ring-0 w-full input"
                               value="{{ request('submission_date_to') }}" />
                    </div>
                    <div>
                        <label for="publication_date_from" class="block mb-1 font-medium text-gray-700 text-sm">Tanggal Publikasi (Dari):</label>
                        <input type="date" name="publication_date_from" id="publication_date_from"
                               class="px-4 py-2 border input-bordered rounded-lg focus:ring-0 w-full input"
                               value="{{ request('publication_date_from') }}" />
                    </div>
                    <div>
                        <label for="publication_date_to" class="block mb-1 font-medium text-gray-700 text-sm">Tanggal Publikasi (Sampai):</label>
