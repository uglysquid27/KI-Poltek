    ...
    <div class="container mx-auto">
                ...
                <div class="relative w-1/7">
                    <button id="dropdownButton" type="button"
                        ...
                        <span id="dropdownSelected">
                            {{ request('filter') ? ucfirst(request('filter')) : 'Paten' }}
                        </span>
                    ...
                    <input type="hidden" name="filter" id="filter" value="{{ request('filter') ?? 'paten' }}">
                </div>
        ...
        <div
            ...
            <div class="flex flex-col justify-center w-full md:w-1/3 px-2 md:px-5">
                <div class="text-xs text-gray-700 font-bold">Nomor Paten</div>
                <div class="font-semibold text-gray-800">{{ $paten->paten_number }}</div>
            </div>
            ...
            <div class="flex flex-col justify-center w-full md:w-1/3 px-2 md:px-5">
                ...
                <div class="font-semibold text-gray-800">{{ $paten->kekayaanIntelektual->submission_date }}</div>
            </div>
            <div class="h-full border-r-1 border-gray-600 mx-2"></div>
            ...
            <div class="flex flex-col justify-center w-full md:w-1/3 px-2 md:px-5">
                ...
                <div class="pt-1">
                    <div
                        ...
                        {{ ucfirst($paten->kekayaanIntelektual->status ?? 'Unknown') }}
                    </div>
                </div>
            </div>
        </div>
        <p class="text-2xl mb-4"><strong> {{ $paten->kekayaanIntelektual->title ?? 'N/A' }} </strong></p>
            ...
            <div class="bg-white shadow-md rounded-lg p-5 mb-3">
                ...
                <div class="grid grid-cols-2 gap-4">
                    <!-- Nomor Paten -->
                    <div>
                        <p class="text-xs text-gray-700 font-bold">Nomor Paten</p>
                        <p class="font-semibold text-gray-800">{{ $paten->paten_number }}</p>
                    </div>
                    ...
                    <div>
                        ...
                        <p class="font-semibold text-gray-800">
                            {{ $paten->submission_date ?? 'N/A' }}</p>
                    </div>

                    <!-- Tanggal Validitas -->
                    <div>
                        <p class="text-xs text-gray-700 font-bold">Tanggal Validitas</p>
                        <p class="font-semibold text-gray-800">
                            {{ $paten->validity ?? 'N/A' }}</p>
                    </div>
                </div>
            </div>
            <div class="p-5 mb-20">
                <div class="grid grid-cols-12 gap-y-15">
                    ...
                    <div class="col-span-10">
                        <table class="table-auto w-full border-collapse border border-gray-300">
                            ...
                            <tbody>
                                @foreach($paten->pemegangs as $pemegang)
                                    <tr>
                    ...
                    <!-- Inventors -->
                    <div class="col-span-2">
                        <p class="text-lg text-gray-700 font-bold">Inventors</p>
                    </div>
                    <div class="col-span-10">
                        <table class="table-auto w-full border-collapse border border-gray-300">
                            <thead>
                            ...
                            <tbody>
                                @foreach($paten->inventors as $inventor)
                                    <tr>
                                        <td class="px-4 py-2 text-lg text-gray-800 w-1/2">{{ $inventor->name }}</td>
                                        <td class="px-4 py-2 text-lg text-gray-800 w-1/2">{{ $inventor->nationality }}</td>
                                    </tr>
                                @endforeach
                            </tbody>
                        </table>
                    ...
                    <div class="col-span-10">
                        <ul>
                            @foreach($paten->konsultans as $konsultan)
                                <li class="font-semibold text-lg text-gray-800">{{ $konsultan->name }} ({{ $konsultan->nationality }})</li>
                            @endforeach
                        </ul>
                    </div>
                </div>
